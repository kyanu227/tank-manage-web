...
import type { ProcurementMode } from "../constants";
import { useProcurementSwipe } from "./useProcurementSwipe";
...
function renderProcurementSwipe(mode: ProcurementMode): void {
  mocks.beginRender();
  ProcurementSwipeHarness({ mode });
  mocks.flushEffects();
}
...
describe("useProcurementSwipe", () => {
  ...
  it("mode更新前の左2連続スワイプを pending target から順に遷移する", () => {
    renderProcurementSwipe("supply-order");
    ...
    expect(mocks.dispatchSwipeEnd.mock.calls).toEqual([
      [{ key: "procurement", committed: true, settledIndex: 1 }],
      [{ key: "procurement", committed: true, settledIndex: 2 }],
    ]);
    expect(mocks.dispatchSwipeProgress.mock.calls).toEqual([
      [{ key: "procurement", baseIndex: 0, offsetTabs: expect.any(Number) }],
      [{ key: "procurement", baseIndex: 1, offsetTabs: expect.any(Number) }],
    ]);
  });
