...
const EXPECTED_CHANGED_FILES = [
  ...
  `${COMPONENT_DIRECTORY}/dashboard-components.test.ts`,
] as const;

const EXPECTED_COMPONENT_FILES = EXPECTED_CHANGED_FILES
  .filter((path) => path.startsWith(`${COMPONENT_DIRECTORY}/`))
  .map((path) => path.slice(COMPONENT_DIRECTORY.length + 1))
  .sort();
...
const EXPECTED_STATE = {
  ...
  logSortOrder: ["DashboardLogSortOrder", "\"desc\""],
  dashboardLoading: [null, "true"],
  isEditMode: [null, "false"],
  selectedLogIds: ["string[]", "[]"],
  editingLog: ["DashboardLogEntry | null", "null"],
  editForm: ["EditForm | null", "null"],
  savingEdit: [null, "false"],
  voidingLog: ["DashboardLogEntry | null", "null"],
  voidReason: [null, "\"\""],
  savingVoid: [null, "false"],
  bulkLocationModalOpen: [null, "false"],
  bulkLocationValue: [null, "\"\""],
  bulkLocationReason: [null, "\"\""],
  savingBulkLocation: [null, "false"],
  bulkVoidModalOpen: [null, "false"],
  bulkVoidReason: [null, "\"\""],
  savingBulkVoid: [null, "false"],
  expandedRootId: ["string | null", "null"],
  historyByRoot: [
    "Record<string, DashboardLogEntry[]>",
    "{}",
  ],
  historyLoadingRoot: ["string | null", "null"],
} as const;
...
const CANONICAL_WRITE_HANDLER_INITIALIZERS = {
  handleSaveEdit: `
    async () => {
...
const CANONICAL_CONTROLLER_HASHES = {
  fetchData: "9a896729",
  refreshAfterCorrection: "87250f",
  openEdit: "73092a7b",
  handleSaveEdit: "2320f2ad",
  handleVoid: "a79ab48",
  toggleEditMode: "8a569103",
  toggleLogSelection: "51abceba",
  selectAllLogs: "e59230d3",
  clearSelectedLogs: "f2976234",
  openBulkLocationModal: "4da2b614",
  handleBulkLocationChange: "d8b7b08d",
  handleBulkVoid: "76cf9c8",
  toggleHistory: "c68a2ce7",
  normalizeCorrectionRole: "68e7bbe",
  toTankActionCode: "7bf12cd2",
  formatActionLabel: "fcab2f56",
  tankStatusColor: "b6e9616d",
  canModifyLog: "117823ef",
  canCorrectLogReason: "c8eccd5",
  canModifyLogReason: "741476b7",
  getEditDisabledReason: "4aeb30c3",
  getVoidDisabledReason: "c2264647",
  getBulkLocationUnavailableReason: "915f7d65",
  timestampToMillis: "64ae8875",
  toDate: "9bbcaaa6",
  formatTime: "8547877b",
  formatReportSource: "39f5e2ba",
  formatReportStatus: "3275173c",
  statusLabel: "ba09139e",
  statusColor: "cc2d2fc6",
  actionBg: "9d8dd271",
  actionFg: "d0dbf65c",
  errorMessage: "329cfc35",
} as const;
...
describe("page/controller AST and source contract", () => {
  it("指定8ファイルとcomponent 7件をexact inventoryとして固定する", () => {
    ...
    expect(EXPECTED_CHANGED_FILES).toHaveLength(8);
    expect(componentFiles).toStrictEqual(EXPECTED_COMPONENT_FILES);
    EXPECTED_CHANGED_FILES.forEach((path) => {
      expect(readSource(path).length, path).toBeGreaterThan(0);
    });
  });

  it("23 stateのname・type・initializerをpageにexactに残す", () => {
    const page = readTypeScriptSource(PAGE_PATH);
    const actual = getUseStateInventory(page);

    expect(actual).toStrictEqual(EXPECTED_STATE);
  });

  it("13 named handlerをpageに残す", () => {
    const page = readTypeScriptSource(PAGE_PATH);

    EXPECTED_HANDLERS.forEach((handlerName) => {
      expect(
        findVariableDeclaration(page, handlerName),
        handlerName,
      ).toBeDefined();
    });
  });

  it("13 handlerとpage helper本文をbase canonical checksumに固定する", () => {
    const page = readTypeScriptSource(PAGE_PATH);

    Object.entries(CANONICAL_CONTROLLER_HASHES)
      .forEach(([name, expectedHash]) => {
        const variable = findVariableDeclaration(page, name);
        const functionDeclaration =
          findFunctionDeclaration(page, name);
        const node = variable?.initializer
          ?? functionDeclaration;

        expect(node, name).toBeDefined();
        expect(
          sourceChecksum(compact(node!.getText(page))),
          name,
        ).toBe(expectedHash);
      });
  });

  it("4 write handler本文をbase canonical sourceと一致させる", () => {
    const page = readTypeScriptSource(PAGE_PATH);

    Object.entries(CANONICAL_WRITE_HANDLER_INITIALIZERS)
      .forEach(([handlerName, expected]) => {
        const declaration = findVariableDeclaration(
          page,
          handlerName,
        );
        expect(declaration?.initializer, handlerName).toBeDefined();
        expect(
          compact(declaration!.initializer!.getText(page)),
          handlerName,
        ).toBe(compact(expected));
      });
  });

  it("query/read model・today二段memo・refresh orchestrationを維持する", () => {
    const source = compact(readSource(PAGE_PATH));

    [
      "const source = await fetchStaffDashboardSourceData()",
      "const entries = await fetchStaffDashboardLogHistory(rootId)",
      "buildStaffDashboardReadModel({ tanks, logs: todayInputs.logs, customerOptions, unfilledReports, staffLocale: todayInputs.staffLocale, nowMillis: todayInputs.nowMillis, })",
      ...
      expect(source).toContain(compact(snippet));
    });
    expect(source).not.toContain(
      compact("nowMillis: Date.now()"),
    );
  });

  it("permission・validation・timestamp・format helperをpageに残す", () => {
    const page = readTypeScriptSource(PAGE_PATH);

    REQUIRED_PAGE_HELPERS.forEach((helperName) => {
      expect(
        findFunctionDeclaration(page, helperName),
        helperName,
      ).toBeDefined();
    });
    expect(compact(readSource(PAGE_PATH))).toContain(
      compact(
  ...
  it("callback adapter・modal confirm wiring・close guardをexactに固定する", () => {
    const source = compact(readSource(PAGE_PATH));
    ...
    [
      ...
      "setBulkVoidReason(\"\"); setBulkVoidModalOpen(true);",
      "const log = logs.find((entry) => entry.id === logId); if (!log) return; openEdit(log);",
      "const log = logs.find((entry) => entry.id === logId); if (!log) return; setVoidingLog(log); setVoidReason(\"\");",
      "const log = logs.find((entry) => entry.id === logId); if (!log) return; await toggleHistory(log);",
      "onConfirm: handleSaveEdit",
      "onConfirm: handleVoid",
      "onConfirm: handleBulkLocationChange",
      "onConfirm: handleBulkVoid",
      "if (savingEdit) return; setEditingLog(null);",
      "if (savingVoid) return; setVoidingLog(null);",
      "if (savingBulkLocation) return; setBulkLocationModalOpen(false);",
      "if (savingBulkVoid) return; setBulkVoidModalOpen(false);",
    ].forEach((snippet) => {
      expect(source, snippet).toContain(compact(snippet));
    });
  });

  it("display projectionを既存helperへ接続しloading中は評価しない", () => {
    const source = compact(readSource(PAGE_PATH));

    [
      "const statusItems = loading ? [] : Object.entries(summary)",
      "label: getLegacyTankStatusLabel(status) ?? status",
      "color: tankStatusColor(status)",
      "const customerLoans = loading ? [] : byLocation.map",
      "const todayOperations = loading ? [] : todayStats.breakdown.map",
      "const reportRows = loading ? [] : recentUnfilledReports.map",
      ...
      "const logRows = loading ? [] : sortedLogs.map",
      ...
      "actionLabel: formatActionLabel(log.action, staffLocale)",
      "actionBackground: actionBg(log.action)",
      "actionForeground: actionFg(log.action)",
      "statusLabel: statusLabel(rev.logStatus)",
      "statusColor: statusColor(rev.logStatus)",
    ].forEach((snippet) => {
      expect(source, snippet).toContain(compact(snippet));
    });
  });
  ...
  it("page/query/read modelのtimestamp helper本文を一致させる", () => {
    ...
    ["timestampToMillis", "toDate"].forEach((functionName) => {
      const printed = paths.map((path) => {
        ...
        if (!declaration) {
          throw new Error(`${path}: ${functionName} missing`);
        }
        return compact(declaration.getText(sourceFile));
      });
  ...
  it("全user-visible copyを新component sourceに保持する", () => {
    const source = EXPECTED_COMPONENT_FILES
      .filter((file) => file !== "dashboard-components.test.ts")
      .map((file) => readSource(`${COMPONENT_DIRECTORY}/${file}`))
      .join("\n");

    USER_VISIBLE_COPY.forEach((copy) => {
      expect(source, copy).toContain(copy);
    });
  });

  it("responsive CSS・class・720pxをViewへ本文不変で移す", () => {
    const source = compact(
      readSource(`${COMPONENT_DIRECTORY}/StaffDashboardView.tsx`),
    );
  ...
  it("component props contractとforbidden import/callをASTで固定する", () => {
    const componentPaths = EXPECTED_COMPONENT_FILES
...
function sourceChecksum(value: string): string {
...
function findFunctionDeclaration(
  ...
  visit(node, (candidate) => {
    if (
      ...
      && candidate.name?.text === functionName
    ) {
      found = candidate;
    }
  });
...
function getUseStateInventory(
  sourceFile: ts.SourceFile,
): Record<string, readonly [string | null, string]> {
  const result: Record<
    string,
  ...
  visit(sourceFile, (node) => {
    ...
    result[stateName] = [
      node.initializer.typeArguments?.[0]?.getText(sourceFile)
        ?? null,
