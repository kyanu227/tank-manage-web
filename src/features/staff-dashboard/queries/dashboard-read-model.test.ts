...
describe("timestamp helper source equivalence", () => {
  it("page・query・read modelのprivate helper本文を同一に保つ", () => {
    const canonicalSource = ts.createSourceFile(
      "canonical.ts",
      `
...
`,
    ...
    const expectedTimestampToMillis =
      printFunction(canonicalSource, "timestampToMillis");
    const expectedToDate =
      printFunction(canonicalSource, "toDate");
    ...
    sourcePaths.forEach((sourcePath) => {
      ...
      expect(
        printFunction(sourceFile, "timestampToMillis"),
        sourcePath,
      ).toBe(expectedTimestampToMillis);
      expect(
        printFunction(sourceFile, "toDate"),
        sourcePath,
      ).toBe(expectedToDate);
    });
  });
...
function readTypeScriptSource(sourcePath: string): ts.SourceFile {
...
function printFunction(
  ...
  if (!declaration) {
    throw new Error(
      `${sourceFile.fileName}: ${functionName} が見つかりません`,
    );
  }
