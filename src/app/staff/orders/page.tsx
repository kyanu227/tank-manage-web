...
function OrdersTab() {
  ...
  if (selectedOrder) {
    ...
    const remaining = Math.max(0, totalRequired - validCount);
    // ヘッダー用サマリー
    //  - 1種別: "スチール10L × 3本"
    ...
    const headerBadgeText = isSingleType
      ...
      : `${selectedOrder.items.length}種・合計${totalRequired}本`;
      ...
      <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ padding: "10px 16px", background: "#fff", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          ...
          <div style={{ flexShrink: 0, display: "flex", alignItems: "baseline", gap: 2 }}>
            <span style={{ fontSize: 24, fontWeight: 900, color: isReady ? "#10b981" : "#3b82f6", lineHeight: 1 }}>{validCount}</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#94a3b8" }}>/ {totalRequired}</span>
          </div>
        </div>
        ...
        {!isSingleType && (
          <div style={{ padding: "8px 16px", background: "#fff", borderBottom: "1px solid #e2e8f0", display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 }}>
            {selectedOrder.items.map((it) => {
              const scanned = scannedByType.get(it.tankType) ?? 0;
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
            ...
            <div style={{
              ...
              <button onClick={fulfillOrder} disabled={!isReady || submitting}
                style={{
                  ...
                  cursor: !isReady ? "not-allowed" : (submitting ? "wait" : "pointer"),
                  pointerEvents: "auto",
