...
export default function OperationsTerminal({ initialMode }: OperationsTerminalProps) {
  ...
  // Master Data
  const { tankMap: allTanks, prefixes, loading: tanksLoading, refetch: refetchTanks } = useTanks();
  const [destinations, setDestinations] = useState<string[]>([]);
  const [destLoading, setDestLoading] = useState(true);
  const loading = tanksLoading || destLoading;
  ...
  /* ═══════════════════════════════════════════════
     Data Fetching
  ═══════════════════════════════════════════════ */
  // tanks 取得は useTanks に集約。ここでは destinations のみを取り扱う。
  ...
  // tanks + destinations を一括再取得（旧 fetchData の挙動を維持）
  const fetchData = useCallback(async () => {
    await Promise.all([refetchTanks(), fetchDestinations()]);
  }, [refetchTanks, fetchDestinations]);
  ...
  // 初回ロード: tanks は useTanks が自動取得するため destinations のみここで取得
