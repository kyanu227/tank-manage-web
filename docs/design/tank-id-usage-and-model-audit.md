...
今後の設計では、`tankId` を自由文字列ではなく、`prefix` と `number` から成る構造化 ID として扱う案が自然である。
...
この設計では、保存上の正本は `"01"` のような数字文字列ではなく、`prefix: "A"` と `number: 1` の組み合わせに寄せる。表示・document id・検索キーとして必要な文字列表現は helper で決定的に生成する。

| 入力 | parse 結果 | canonical / display |
|---|---|---|
| `A1` | `{ prefix: "A", number: 1 }` | `A-01` |
| `A01` | `{ prefix: "A", number: 1 }` | `A-01` |
| `A-01` | `{ prefix: "A", number: 1 }` | `A-01` |
| `A001` | `{ prefix: "A", number: 1 }` | `A-01` |
| `A100` | `{ prefix: "A", number: 100 }` | `A-100` |
...
`prefix` と `number` を source data として扱う場合、次のルールを基本にする。
...
`formatTankId("A", 1)` は `A-01` を返す。数字部分は最低2桁ゼロ埋めにするが、3桁以上はそのまま表示する。
...
自然順ソートは文字列ではなく、`prefix` と `number` に分解して行う。
...
6. `A-OK` を正式な tank ID 形式として残すか、旧互換扱いにするか。
