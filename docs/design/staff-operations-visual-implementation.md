...
- 塗りは **送信ボタンだけ**。A-OK は淡い面の ID ディスプレイへ降りる
...
| shadow | 全領域に付けない。送信 `0 12px 24px -12px accent95%` ＋ 1px 影 / Commit `0 12px 22px -20px` / Drum は inset のみ |
...
- 送信は枠の内側下辺 8px・48px・radius 13。上に 22px の fade、リストは下端 padding 70px
...
| 調整 | 理由 |
|---|---|
| **画面全体のパステル配色を採用しない**。背景は shell と同じ `#f4f6fa` の 1 枚で、mode ごとの緑・暖色の色被せもしない。沈みは下部の `rgba(15,23,42,.032)` だけ | 利用者の明示指示（画面全体がうっすらパステルなのをやめる）。正本の「面は 1 枚 / 白帯・灰帯を作らない」意図は、色を足さずに満たせる。accent は送信・ドラム選択・チップなど小面積だけが持つ |
| 背景 gradient を `#f7f9fc → #e9eef7` ではなく shell 基準色から始める | ヘッダー／タブ帯（`rgba(252,253,255,.78)` + blur）との境目に帯が出るのを避けるため。上端の色を shell と一致させることで、正本が求める「継ぎ目のない 1 枚」に実際になる |
...
| DrumRoll の新しい見た目を `variant="soft"` の opt-in にした | `TankIdInput`（破損 / 修理 / 検査 / 自社）も同じコンポーネントを使う。今回の対象外画面の見た目を巻き込まないため。統一は別 PR で |
...
1. 上端から下端まで単一の背景（白帯・灰帯・区画ごとの塗り分けを作らない）
