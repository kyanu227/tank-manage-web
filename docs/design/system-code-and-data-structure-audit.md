...
この文書は、コード構造・業務責務・`logs` / `tanks` / `transactions` を横断した全体監査であり、今後の再設計では主資料として扱う。

`logs` / `tanks` 個別の詳細監査は、別 PR #85 の `docs/design/logs-and-tanks-data-structure-audit.md` を補助資料として扱う。次フェーズでは、この文書を入口にし、必要に応じて PR #85 の詳細監査を参照する。
