...
- `empty → system fill → operator lend/order_lend`
- `lent/unreturned → system return → system fill → operator lend/order_lend`
- `in_house → system inhouse_return → system fill → operator lend/order_lend`
...
返却申請の確定では、transaction内で申請`customerId`、tankの現在holder `customerId`、
操作contextの`customerId`を再読取して完全一致を要求する。欠落・不一致時は全件を中止し、
return transactionを完了しない。完了transactionは同atomic requestで生成した`fulfilledLogId`を持つ。

## Atomic上限とRules

`MAX_ATOMIC_TANK_OPERATIONS = 10`。通常のadvisory recoveryは100件でもRulesを通過するが、
返却transactionとtank/logを相互結合する最も重い経路は10件までを安全上限とする。
上限超過時は自動分割せずエラーにする。
...
1 / 10 return transaction coupled operation: PASS
50 / 100 return transaction coupled operation: Rules上限でDENIED
