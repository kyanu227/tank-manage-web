...
- direct: 外部顧客への正規貸出・返却を含め、`transitionReviewStatus = not_required`
- recovery: 状態snapshotは即時更新する。`transitionPlan.steps`にcustomerId付きの
  `rental_open` / `rental_close`があるか、`hasUnknownAffectedCustomer == true`なら`pending`。
- 外部顧客の貸出サイクルを変更しないinternal-only recoveryは`not_required`。
...
projectOfficialAggregationEvent direct / recovery + not_required / recovery + approvedの最終operator step
```

外部顧客に影響するpending recoveryは影響顧客の請求書印刷を停止する。
影響顧客不明なら全件停止する。internal-only recoveryの`not_required`は停止対象にしない。
...
internal-only recoveryの`not_required`作成、`pending → approved`、direct operation、direct logのcorrection、
正式集計対象logのvoidでは
正式集計revisionも増やす。
...
- 再実行がrecoveryになる場合は、再計画したtransitionPlan、確認したevidence、再生成したfingerprintを
  持つ別logを作成する。外部顧客影響ありまたは影響先不明なら`pending`、
  internal-onlyなら`not_required`とする。
...
初回のtransitionPlan v1 cutoverは2026-07-18に完了済み。Data Reset、schema verification、
runtime strictのHosting、通常Rules、strict smokeまで反映し、production Reset / restore gateは
再度closedに戻した。2026-07-19の管理者review条件改訂ではData Resetを行わない。

初回cutoverとその後のadvisory有効化順序（maintenance window終了まで2026-07-18実施済み）:
...
暗号化snapshotを唯一の正本として、`scripts/reset-transition-cutover-snapshot.ts`が
