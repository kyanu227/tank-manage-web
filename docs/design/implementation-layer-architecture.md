...
### 1.1 Guidance, Not a Rigid Template

This architecture is guidance and decision criteria, not a rigid mandatory layer template.

Codex は、意図した境界を守れる範囲で最も単純な構造を選ぶ。全ての変更に page / hook / command / service / repository / query / stats を機械的に揃える必要はない。

判断基準:

- 小さく低リスクな UI / settings 変更は過剰設計しない。
- 単一 settings document の保存や単純な master data 更新は、薄い service または repository adapter で足りる場合がある。
- 高リスク write、複数 collection write、tank / logs lifecycle transitions、auditability、future reuse、security-sensitive operations は、service / query / stats 境界を明確にする。
- 既存 feature の形が十分に薄く保てる場合は、無理に新しい layer や folder を増やさない。
- 境界を増やす目的は、責務を追えるようにすること、将来の履歴・通知・権限・検証を差し込めるようにすることであり、folder 構成を増やすこと自体ではない。
...
- admin-owned write UI の form / button / confirmation / screen-level composition
...
ここでいう「page は Firestore に直接 write しない」は、Admin 画面に保存ボタンや登録フォームを置いてはいけない、という意味ではない。

Admin page は、管理者が操作する write-capable UI を持ってよい。例えば staff 登録、staff 情報更新、金額・単価設定、顧客設定、発注品目設定、管理者権限設定などは admin-owned write であり、Admin 画面に UI action として存在してよい。

避けるべきなのは、page が Firestore SDK を直接 import して `addDoc` / `setDoc` / `updateDoc` / `writeBatch` の詳細を抱えたり、複雑な write orchestration を page 内に埋め込むことである。Admin page は UI action、form、button、confirmation、screen-level composition を持ち、実際の write logic はリスクと複雑さに応じて hook / service / repository boundary へ流す。
...
- page direct write 禁止は、write-capable UI 禁止ではない。
...
Admin page の write UI flow:

```text
admin page
  -> admin hook
    -> admin service
      -> repository / settings adapter
        -> Firestore SDK
```

小さい設定変更では `admin hook -> thin service -> settings adapter` 程度でよい。複数 collection 更新、権限、actor 解決、将来履歴が関わる場合は service 境界を厚めにする。
...
- 発注品目設定 UI
- 管理者権限設定 UI
- 耐圧 / portal / notification settings UI
- 保存 button / confirmation / error display

Admin page が持たないもの:

- Firestore SDK の import
- `addDoc` / `setDoc` / `updateDoc` / `writeBatch` の直接呼び出し
- staff / staffByEmail mirror sync の詳細
- 複数 collection consistency の orchestration
- 将来 `edit_history` を差し込むべき write の詳細
...
admin-owned write examples:

- staff registration
- staff profile update
- staff / staffByEmail sync
- customer settings
- price / rate registration
- order master settings
- admin permissions
- inspection / portal / notification settings
...
### 4.3 Service Depth Criteria

service 境界は、変更のリスクと複雑さに応じて厚みを変える。

service を厚くするべき条件:

- multiple collection writes
- staff / customer identity resolution
- staffByEmail or mirror sync
- permission-sensitive writes
- price / rate changes that may affect billing interpretation
- future audit / edit history insertion point
- tank / logs lifecycle or business invariant risk
- notification side effects or future notification insertion point
- Security Rules と service validation の対応関係を明確にしたい write

service を薄くしてよい条件:

- single settings document update
- low-risk display preference or simple master data update
- repository helper already encapsulates Firestore adapter details
- no lifecycle transition
- no cross-collection consistency issue
- no audit requirement
- no permission-sensitive branching beyond the page guard

薄い service は、単なる pass-through に近くてもよい。目的は layer を増やすことではなく、page から Firestore SDK の詳細を外し、将来必要になったときに validation / history / notification を差し込める呼び出し点を作ることである。

### 4.4 Repository Write Helpers
...
### 4.5 Security Rules and Service Validation
...
Admin-owned management writes と report handling は別物である。

- Admin-owned management writes は許容する。
- staff 登録、staff 情報更新、顧客設定、単価設定、order master、admin permissions、portal / notification settings は admin-owned write UI として扱える。
- ただし portal unfilled report handling は別であり、初期 Phase では admin が handling owner ではない。
- unfilled report の handling metadata は staff が owner であり、`handlingStatus` / `handledBy...` / `handlingNote` を中心にする。
- admin management settings の write を、admin が report handling を行うことと混同しない。
