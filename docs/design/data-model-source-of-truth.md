...
| `logs` | tank lifecycle と関連業務の履歴・監査 | 過去に発生した操作 event、状態遷移記録、revision / void chain | staffName / customerName / location の当時 snapshot | 現在状態の source of truth にはしない |
...
- tank lifecycle history / past operation events: `logs`
...
`tanks` はタンクの現在状態 snapshot の source of truth とする。通常の画面表示や業務処理は、現在状態を毎回 `logs` から復元するのではなく、`tanks` の current projection を読む。
...
Current loan projection means "where should the app read the current loan state from?" It is not a new source competing with `logs`; it is a current-state read model on `tanks` that can be backed by lifecycle logs and restored by snapshots. This PR only records the design candidate and does not add any field.
...
`logs` is the source of truth for past tank lifecycle events and audit trails. It records what happened, when it happened, who did it, and what tank snapshot changed. It is not the source of truth for the current tank state.
...
| operation category | `transitionAction` | past-event classification source of truth candidate |
| display operation name | `action` | display snapshot / compatibility |
| status before / after | `prevStatus`, `newStatus` | past-event audit source of truth |
...
`logs` may store enough snapshot data to explain what happened and to restore a tank after correction / void, but `tanks` remains the current-state read model.
...
|---|---|---|
| Loan date ambiguity | UI uses `tanks.updatedAt` as lend date approximation | add current loan projection or derive a read model from lifecycle logs |
