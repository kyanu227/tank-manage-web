...
- Phase 14: locale selection design docs
...
- Phase 20: manual operation labels
- Phase 21: ReturnTag label helper only
- Phase 22: ReturnTagSelector label application
- Phase 23: return UI display-only fixed labels
- Phase 24: operation message dictionary design docs
- Phase 25: operation message helper only
- Phase 26: manual operation confirmation / success messages
- Phase 27: return processing messages
- Phase 28: staff locale setting messages
- Phase 31: ACTION / STATUS code pivot design docs
- Phase 32: code type preparation
- Phase 33: OP_RULES code pivot
- Phase 34: tank-operation write code pivot
- Phase 35: query pivot
- Phase 36: aggregation pivot
- Phase 37: legacy cleanup
- Later: customerId / location cleanup
- Later: collaborators / payout

このサブロードマップでは、Firestore 保存値、Firestore query、`OP_RULES`、`tank-operation.ts` は変更しない。まず UI label と legacy 日本語保存値を分離する。

locale の正本は staff profile の `staff.locale` とする。localStorage は正本ではなく、使うとしても補助キャッシュに限定する。

操作確認文・成功文・エラー文は、単語 label ではなく変数を含む message として扱う。詳細方針は `docs/i18n-operation-message-policy.md` を正本とする。message 多言語化と Firestore 保存値 code 化は混ぜない。

ACTION / STATUS code pivot の詳細方針は `docs/action-status-code-pivot-design.md` を正本とする。既存データ互換や legacy backfill は不要だが、`OP_RULES`、`tank-operation.ts`、query、trace、billing、sales、staff analytics への依存を確認しながら分割実装する。
