...
| `pending_approval` | order 側に残る可能性がある旧 status。return 側では使わない | order 側の既存互換を保つ | 旧 order flow | order は `approved` | order 処理時に logs | order type のみ | order に関係 | return の処理待ちとして使わない | return 側は `pending_return` に一本化済み。order 側整理は別PR |
| `pending_return` | 顧客返却タグの処理待ち | return を承認申請ではなくタグ処理待ちとして扱う | portal return / CustomerUser | `completed` | 処理完了時に logs | finalCondition, fulfilledBy* | 返却処理に関係 | order に使わない。顧客がタグを付けた時点で tank status を変えない | 正しい |
...
- `pending_approval` は return 側では使わない。return 側は顧客返却タグの処理待ちとして `pending_return` に一本化済み。order 側の整理は別 PR。
