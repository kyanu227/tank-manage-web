...
| customerName | Customer 紐付け後の請求・履歴表示用 snapshot |
...
`transactions.customerName` は Customer 紐付け後の請求・履歴表示用 snapshot としてのみ使う。
未紐付け時の自己申告名は `requestedCompanyName` に保存し、`customerName` には入れない。

Customer 紐付け時に service 側で次の表示名を作る。

```ts
const customerDisplayName = customer.companyName?.trim() || customer.name.trim();
```

この `customerDisplayName` を `transactions.customerName` に snapshot する。
理由は、請求・履歴表示では個人名より会社名・店舗名を優先する方が自然だからである。
`companyName` が未設定または空文字の場合のみ `name` を使う。
...
- `customerDisplayName` を作る責務は page ではなく service に置く。
...
申込時に CustomerUser が入力した会社名・氏名・LINE 名と、後で紐付けた Customer の正式な表示名を比較できるようにする。
...
- dashboard の要対応件数に `pending_link` を混ぜる場合は、通常受注とは別の「未紐付け」件数として扱う。
...
`customerDisplayName` は `linkedCustomer.companyName || linkedCustomer.name` で作る。
...
- `customerDisplayName = customer.companyName || customer.name` を作る。
...
- `customerName` には `customer.companyName || customer.name` の snapshot が入る。
...
6. customerLinkingService
   ...
   - `customerName` を `companyName || name` に固定する。
7. admin 紐付け処理への接続
