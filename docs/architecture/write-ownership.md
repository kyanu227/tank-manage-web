...
2. tank lifecycle（tanks + logs + tankAggregationRevision）は `src/lib/tank-operation.ts` のみが書く
...
|---|---|---|
| status / location / staff / note / latestLogId / nextMaintenanceDate / customerId / customerName | `applyTankOperation` / `applyBulkTankOperations` / `commitPlannedOperations`（tank-operation.ts:364-675）、訂正 `applyLogCorrection`（:817-1030）、取消 `voidLog`（:1036-1095） | 新規タンク作成時のみ `submitTankEntryBatch`（procurement）も可 |
| customerId / customerName | 同上 | 現在貸出projection（tank-types.ts:9-12）。**顧客identityの正本ではない**（正本はcustomers / logs）。operation経由以外の書き込み禁止 |
| logNote | ①operation時のtankNote反映: tank-operation.ts:601-618 ②返却tag marker単独write: `updateTankReturnTagMarker`（tank-tag-service.ts:5-17） | **二重owner（暫定容認）**。②の呼び出し元は useBulkReturnByLocation と inhouse page（将来は各workflow service経由）。返却確定前の一時tag stateとしての利用（R-17）は機能干渉リスクとして認識済み。解消はschema変更を伴うため構造化PRと分離した別設計 |
...
| type | 作成 | 更新・確定 |
|---|---|---|
| order | `createPortalOrder`（portal-transaction-service.ts:42-71） | pending_link更新 `linkCustomerUsersToCustomers`（customer-linking-service.ts:73-147、customerUsersとbatch）/ 承認・完了 `approveOrder` / `fulfillOrder`（order-fulfillment-service.ts:14-70、operationとatomic） |
...
| staffByEmail | mirror helper `setStaffAuthMirrorInBatch` / `deleteStaffAuthMirrorInBatch`（staff-auth.ts:49-68）。呼び出し元は staff-sync-service:63-98 と staff-locale-service:55-64 の2経路（R-09）。一本化は後続候補（意味変更なしで可能な場合のみPR化） |
...
| tankProcurements | `submitTankEntryBatch`（procurement/lib:54-83） |
...
| 受注承認・完了 | operation + transaction完了（order-fulfillment-service） |
| 返却確定 | 再取得 + 変換 + operation + transaction完了（return-tag-processing-service） |
...
| procurement | tankProcurements + tanks + logs（batch、submitTankEntryBatch） |
