...
`cutover:readiness`がGOであることをcutover operatorが明示確認する。今回は一人運用とし、
別reviewer principalを必須としない。
...
一回限りのoperator confirmationと一緒に追加する。現時点のCLIは`--execute`を指定してもproductionでは
...
point-of-no-return前は、freezeとwriter停止を維持したまま、暗号化snapshotからrestoreを計画し、将来の明示的な
