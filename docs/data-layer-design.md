...
- `listenPendingReturnTags(callback)` — 返却タグ処理待ち購読。
...
- `getPendingTransactions()` — `status in ["pending", "pending_approval", "pending_return"]` 横断取得（admin ダッシュボード用）。`pending_link` は通常の要対応件数に含めない。
- `findPendingLinksByUid(uid)` — `createdByUid == uid && status == "pending_link"` の特殊条件取得（admin/settings の顧客リンク処理用）。
...
- 影響が小さい場所から repository の読み出し関数に寄せる。
  ...
  - 例: `useReturnTagProcessing` / `useOrderFulfillment` の読み取り部分 → `transactionsRepository`
