...
| `pending_approval` | 変更なし | return では `pending_return` 候補 | 現行互換名。order と同じ意味にしない |
...
実施状況: PR #14 で実施済み。
`pending_approval` status migration は未実施。
...
方針は、まず読み取り互換で `pending_approval` と `pending_return` の両方を表示し、新規作成を後続で `pending_return` へ切り替え、既存未処理 return は原則として自然消化することである。
