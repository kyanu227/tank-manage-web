...
現行コード名は、後続 PR まで残る。
AI / Codex は、以下の読み替えを前提にコードを読む。

| 現行コード名 | 現行の場所 | 業務意味として読む名前 | 補足 |
|---|---|---|---|
| `ReturnApprovalScreen` | `src/features/staff-operations/components/ReturnApprovalScreen.tsx` | `ReturnTagProcessingScreen` | 顧客返却タグを処理する画面 |
| `useReturnApprovals` | `src/features/staff-operations/hooks/useReturnApprovals.ts` | `useReturnTagProcessing` | pending return tags の取得と処理 |
| `fetchApprovals` | `useReturnApprovals` 内 | `fetchPendingReturnTags` | 処理待ちの顧客返却タグを取得する |
| `fulfillReturns` | `useReturnApprovals` 内 | `processReturnTags` | 選択した返却タグに基づき tank operation を実行する |
| `pending_approval` | `transactions.status` | return では `pending_return` 候補 | 現行互換名。order と同じ意味にしない |

`approval` という名前は、現行コード名として参照するだけにする。
業務意味として「顧客申請の承認」と解釈しない。

## 4. transaction status 方針

現行では return transaction の処理待ち status として `pending_approval` を使っている。

ただし、return における `pending_approval` は承認待ちではない。
業務意味としては、顧客返却タグの処理待ちである。

後続で status 名を固定する場合は、第一候補を `pending_return` とする。
...
そのため、status は `pending_return`、画面・hook・関数名は `ReturnTagProcessing` 系に分ける方針を推奨する。
...
この PR では `pending_approval` はまだ維持する。
import / export / component 名の rename と、画面内の表示文言整理だけに絞る。

### PR B: return status migration design

目的は、return transaction 側だけ `pending_approval` を `pending_return` へ移行する設計を固めること。
ここでは既存データ互換、query、admin dashboard、staff dashboard、repository 型への影響を確認する。

候補:

- 新規 return transaction は `pending_return` で作る。
- 読み取りは移行期間だけ `pending_approval` と `pending_return` の両方を見る。
- 既存 `pending_approval` return を一括更新するか、自然消化するかを決める。
...
- `src/**` の変更。
- `pending_approval` の status migration。
- `ReturnApprovalScreen` / `useReturnApprovals` の rename。
...
既存 docs に `返却申請`、`返却承認`、`approval` 系の現行コード名が残っている場合でも、それは現行実装を指す名前である。
