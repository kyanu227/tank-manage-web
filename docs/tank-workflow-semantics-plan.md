...
- `不良` を status として残すか、破損報告タグへ寄せるか。
- `耐圧検査中` を正式な tank status として追加するか、検査対象一覧だけで制御するか。
...
現時点では、タンクが倉庫にあるか、顧客が保持しているか、自社利用中かを表示するための「現在場所・現在保持者名」として扱う。
将来的には `currentHolderName`、`currentLocationName`、`customerId` などへ分離する候補である。
...
ただし業務名としては強すぎる。
後続では `return request` / `return tag` / `pending_return` のように、承認ではなくタグ付けであることが分かる名前へ見直す候補である。
...
| `不良` | 名前が曖昧。未充填や準備不備と混同しやすい | status として残すか再検討。破損報告タグへ寄せる候補 |

`不良` は「未充填」と混同しやすい。
未充填はタンクの状態異常ではなく、充填されていない状態で貸し出されたという業務上の不備である。
一方、バルブが固い、空気漏れ、外傷があるなどはタンク自体の不具合であり、`破損` または破損報告タグで扱う方が自然である。
...
推奨は、人間が業務で理解しやすい `持ち越し`。
ただし、ダッシュボードや集計では `未返却` の方が状態名として分かりやすいため、action と status の名前は分けてよい。
...
| 不良 | 名前が曖昧。タンク不具合、未充填、準備不備のどれにも読める | 廃止または名称変更候補 |
...
`不良` を残す場合でも、少なくとも未充填とは別物として扱う。
新規設計では `不良` という status 名を増やすより、`破損` + タグへ寄せる方が安全である。
...
後続では、`RETURN_TAG.DEFECT` / `RETURN_DEFECT` の名称を `UNCHARGED` または `UNFILLED` 系へ見直す候補とする。
ただし名称変更は、報酬・請求・trace・UI 文言への影響があるため単独 PR にする。
...
| 持ち越し / 未返却化 | `貸出中` | `未返却` | 顧客が持ち越す、または返却予定を過ぎた状態にする |
| 充填 | `空` | `充填済み` | 通常の充填 |
...
| 修理済み | `破損` | `空` | `不良` を残す場合だけ `不良` も許可候補 |
| 耐圧検査完了 | `耐圧検査中` または検査対象一覧で制御 | `空` | 現行 status に `耐圧検査中` はないため要設計 |
...
- `STATUS.DEFECTIVE` / `不良` があるが、業務意味が曖昧。
- `STATUS.UNRETURNED` / `未返却` はあるが、`貸出中 -> 未返却` の明示 action がない。
...
現時点では「現在場所・現在保持者名の表示用文字列」と定義し、配達先 `deliveryTargetName` とは分ける。
将来は `customerId`、`currentHolderName`、`currentLocationName` への分離を検討する。
...
本書を読んだ上で、人間側に確認したい点。
ここが確定すると、後続 PR の実装がさらに安定する。

1. `持ち越し` と `未返却化` の action 名はどちらを正にするか。
   - 業務画面では `持ち越し`、status は `未返却` が自然に見える。
2. `耐圧検査中` を正式な tank status として追加するか。
   - 追加しない場合は、検査対象一覧やメンテナンス画面側の gate で制御する必要がある。
3. `不良` status は廃止候補でよいか。
   - 推奨は `破損` + タグへ寄せること。
4. return request の新しい status 名をどうするか。
   - 候補: `pending_return`, `tagged`, `open`, `pending_processing`。
   - ただし英語 status は Firestore query に残るため、実装前に固定したい。
5. 未充填返却の action 名は `返却(未充填)` のままでよいか。
   - `RETURN_DEFECT` という内部名は誤解されやすいため、内部名だけでも `RETURN_UNFILLED` / `RETURN_UNCHARGED` へ寄せる候補。
6. `location` の将来名は `currentHolderName` と `currentLocationName` のどちらが業務に近いか。
   - 顧客保持を表すなら `currentHolderName`、倉庫・自社・顧客を含む場所表示なら `currentLocationName` が近い。
7. 貸出中に顧客から破損連絡があった場合、すぐ `破損` status にするか、まず報告記録だけ作って回収後に `破損` status にするか。
   - 本書の推奨は、貸出中 / 未返却中は直接 status を変えず、報告記録を残して回収後に状態変更すること。
...
この PR では、`pending_approval` の削除、`不良` の削除、`location` rename、return service 再設計を混ぜない。
