...
ただし業務名としては強すぎる。
現行コード名として `ReturnApprovalScreen` / `useReturnApprovals` / `fetchApprovals` / `fulfillReturns` が残っていても、業務意味は「スタッフによる返却タグ処理」と読む。
...
現行では `transactionsRepository.getReturns({ status: "pending_approval" })` を読み、スタッフが「承認」する UI 名になっている。
ただしこれは現行コード名であり、業務上は顧客が付けた返却タグを現場回収時に処理する画面である。
...
3. return transaction は承認申請ではなく、顧客が貸出中タンクへタグを付ける補助システムとして扱う。
   - 現行コード名の `ReturnApprovalScreen` / `useReturnApprovals` は旧名として読み替える。
