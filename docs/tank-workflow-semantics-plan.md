...
- `pending_approval`、`DEFECTIVE`、旧返却タグ名、`location` などの名前から勝手に意味を補完しない。
- 本書で「廃止候補」「名称変更候補」「後続 PR」と書いたものを、同じ PR で実装しない。
- UI 文言だけを直して業務設計を変えたことにしない。
- 状態遷移を変える場合は、`tank-rules.ts` だけでなく、`tank-operation.ts`、報酬、請求、返却タグ処理、ポータル transaction への影響を確認する。
...
| `docs/return-tag-processing-naming-design.md` | return transaction とスタッフ側処理の旧 approval 命名の読み替え正本 |
...
`type = "return"` は、業務上は「返却申請」や「返却承認申請」ではなく、顧客が現在貸出中のタンクへ返却時の扱いタグを付ける補助情報である。
本書ではこれを「顧客返却タグ transaction」として扱う。

主目的:

- 顧客が返却予定のタンクを事前に示す。
- タンクごとに `normal` / `unused` / `uncharged` / `keep` のタグを付ける。
- スタッフが現場で回収・処理する時の確認材料にする。

旧実装では `status: "pending_approval"` を使っていた。
PR #14 で画面・hook 名は `ReturnTagProcessing` 系へ寄せ、PR #17 で return 側の処理待ち status は `pending_return` に一本化済みである。
旧コード名として `ReturnApprovalScreen` / `useReturnApprovals` / `fetchApprovals` / `fulfillReturns` が出てきた場合も、業務意味は「スタッフによる返却タグ処理」と読む。
PR #14 で screen / hook / function 名は `ReturnTagProcessing` 系へ rename 済みである。
`ReturnTagProcessing` / `pending_return` のように、承認ではなくタグ処理であることが分かる名前を正とする。
status 名は `pending_return` を正とする。
理由は、「返却処理待ち」と読め、`pending` や `pending_link` と混同しにくいためである。
`tagged` は何のタグか分かりにくく、`open` は汎用すぎる。
`pending_processing` は業務意味が曖昧である。
PR #15 で `docs/pending-return-status-migration-design.md` を追加し、return 側だけ `pending_return` へ移行する設計を固定済みである。
PR #16 では一時的に両 status 読み取りを入れたが、本運用前・未実装扱いのため PR #17 で不要として整理し、return 側は `pending_return` に一本化済みである。
...
- 顧客返却タグ transaction は tank の状態を直接変えない。
...
| `不良` | 名前が曖昧。未充填や準備ミスと混同しやすい | 新規設計では廃止方針。破損報告タグへ寄せる |

`不良` は「未充填」と混同しやすい。
未充填はタンクの状態異常ではなく、充填されていない状態で貸し出されたという充填ミスの記録である。
...
| 不良 | 名前が曖昧。タンク不具合、未充填、準備ミスのどれにも読める | 新規設計では廃止方針 |
| 未充填 | 充填されていないタンクが貸し出された報告。こちら側の充填ミス | `uncharged_report` transaction として記録し、破損 status にしない |
...
旧内部名では、未充填返却をタンク自体の不良・破損に見える名前で表していた。
現在は uncharged 系の名前へ揃え、未充填と破損を分離する。
...
| 通常返却 | 使用済みとして戻る | `空` |
| 未使用返却 | 使用されずに戻る | `充填済み` |
| 未充填返却 | 未充填として戻る。タンク故障ではない | `空` |
| 持ち越し | 顧客が使わずに保持し続ける | `未返却` |

返却時の未充填は `RETURN_TAG.UNCHARGED` / `ACTION.RETURN_UNCHARGED` を正とする。
既存の `uncharged_report` と同じ uncharged 系の名前へ揃え、破損・不良と混同しないようにする。
...
| 自社返却(未充填) | `自社利用中` | `空` | 自社利用中の未充填返却。破損とは分ける |
...
- 返却時の未充填は `RETURN_TAG.UNCHARGED` / `ACTION.RETURN_UNCHARGED` へ整理済み。

後続 PR では、`allowedPrev: []` を「本当に制限なしでよいか」ではなく、業務上の安全条件から再定義する。

### `src/features/staff-operations/hooks/useReturnTagProcessing.ts`

現行では `transactionsRepository.getPendingReturnTags()` が `pending_return` の顧客返却タグを読み、スタッフが現場回収時に処理する。
PR #14 で旧 `useReturnApprovals` / `ReturnApprovalScreen` は `useReturnTagProcessing` / `ReturnTagProcessingScreen` へ rename 済みである。
詳細な読み替えは `docs/return-tag-processing-naming-design.md` を正とする。

後続では、service 境界や dashboard 表示の整理を候補とする。
ただし、返却タグ処理の挙動を変える PR では `tank` 状態遷移と `transactions` 完了更新を同時に確認する。
...
3. return の旧 approval 命名、status 名、service 境界を整理する。
4. `uncharged_report` を「記録」として明確化し、対応待ち status にしない方針を守る。
5. damage report にタグ設計を追加する。
6. `location` / current holder / `customerId` の read/write 境界を整理する。
7. 報酬・請求・trace への影響を、状態遷移変更とは別 PR で確認する。
...
- 顧客返却タグ transaction 作成だけでは tank status が変わらない。
...
3. return transaction は承認申請ではなく、顧客が貸出中タンクへタグを付ける補助システムとして扱う。
   - 旧コード名の `ReturnApprovalScreen` / `useReturnApprovals` は PR #14 で `ReturnTagProcessingScreen` / `useReturnTagProcessing` へ rename 済み。
   - 業務意味は「スタッフによる返却タグ処理」である。
   - 新しい status 名を固定するなら `pending_return` を推奨する。
4. 返却時の未充填は uncharged 系の名前を正とする。
   - `RETURN_TAG.UNCHARGED` / `ACTION.RETURN_UNCHARGED` を使う。
   - 自社返却側も `ACTION.IN_HOUSE_RETURN_UNCHARGED` を使う。
