...
現行コードでは `status: "pending_approval"` を使っている。
PR #14 で画面・hook 名は `ReturnTagProcessing` 系へ寄せたが、status 名は現行互換として残している。
...
後続では `ReturnTagProcessing` / `pending_return` のように、承認ではなくタグ処理であることが分かる名前へ見直す。
...
実装順は、まず `pending_approval` と `pending_return` の読み取り互換を入れ、その後に新規 return 作成 status を `pending_return` へ切り替える。既存未処理 return は原則として一括更新せず、読み取り互換で自然消化する。
...
現行では `transactionsRepository.getReturns({ status: "pending_approval" })` を読み、スタッフが顧客返却タグを現場回収時に処理する。
