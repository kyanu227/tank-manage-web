...
含まれるが、rollout gateは無効のままであり、保存済みpolicyと実行時policyはいずれもstrictである。
...
- 個別・一括印刷: 候補がprint-readyで両PDF保存ボタン有効、print block 0件
...
- cutover失敗時に暗号化snapshotから一括復元できる。

## Rollback情報

- データ: 上記暗号化snapshotとKeychain鍵を使い、reset直後の期待状態からatomic restoreする。
...
- advisory rollout gateの有効化
- policyをadvisoryへ変更
- advisory recoveryの本番smoke
- 管理者reviewの承認 / 除外smoke
- 承認前後の請求・売上・実績反映確認
- migration SA / Rules reader SAの期限付きbindingの剥奪または期限切れ確認
- snapshotとKeychain鍵の保持期限決定、および不要後の安全な削除
