...
| `useReturnTagProcessing.fetchPendingReturnTags` | `transactionsRepository.getPendingReturnTags()`（内部は `pending_return`） | 2-B-8a / PR #17 |
| `src/app/admin/page.tsx` 要対応件数 | `transactionsRepository.getPendingTransactions()`（`pending` / `pending_approval` / `pending_return`、`pending_link` は除外） | 2-B-9 / PR #18 |
| `src/app/staff/dashboard/page.tsx` pending orders | `transactionsRepository.getOrders({ status: "pending" })` | 2-B-10a |
| `src/app/staff/return` 返却タグ処理待ち | `transactionsRepository.getPendingReturnTags()`（内部は `pending_return`） | PR #17 |
...
| X2 | `useReturnTagProcessing.ts` | L44 | `getDocs(query, type=="return", status=="pending_return")` | 表示用 | `getPendingReturnTags()` |
| X3 | `admin/page.tsx` | L48 | `getDocs(query, status in ["pending","pending_approval","pending_return"])` | 表示用 | `getPendingTransactions()` |
| X4 | `staff/dashboard/page.tsx` | L145 | `getDocs(query, type=="order", status=="pending")` | 表示用 | `getOrders({ status:"pending" })` |
| X5 | `staff/return` 返却タグ処理待ち | L44 | `getDocs(query, type=="return", status=="pending_return")` | 表示用 | `getPendingReturnTags()` |
...
- 新規候補: `getPendingTransactions()` — orders + returns 横断（admin/page 用）
  もしくは画面側で `getOrders({status:"pending"})` と `getPendingReturnTags()` を Promise.all
  → 実装: `getPendingTransactions({ statuses })` で type 横断 status `in` 配列（既定は `pending` / `pending_approval` / `pending_return`、`pending_link` は含めない）
